...
const isAuthenticated = asyncHandler(async (req, res) => {
  try {
    ...
    const { accessToken, refreshToken } = req.cookies; // Or whatever you named your cookie

    if ((!accessToken && !refreshToken) || !refreshToken) {
      // No token found, so the user is definitely not logged in.
      // This is not an "error" state, it's a valid status.
      return res
        .status(401)
    ...
    if (!accessToken) {
      ...
      if (!user) {
        return res
          .status(401)
          .json(
            new apiResponse(401, {}, 'user not authorized try login again')
          );
      }
      const newAccessToken = jwt.sign(
        {
          ...
          role: user.role, // The role is now a direct property
        },
    ...
    // 3. (Optional but recommended) Check if the user still exists in the DB
    const user = await User.findById(decodedAccessToken._id).select('-password'); // Exclude password from result

    if (!user) {
      return res.status(401).json({ message: 'Unauthorized: Try login again' });
    }
    // 4. If everything is okay, send the user data back
    res.status(200).json(new apiResponse(200, user, "user verrified successfullly"));
  } catch (error) {
